...
export function playPinchSnapSound() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    ...
    // High tech snap / magnetic latch click
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'triangle';
...
export function playInvertChord() {
  if (!soundEnabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    ...
    // Harmonic two-tone chord for viral upside-down mode
